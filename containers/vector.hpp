...
public:
	typedef T											value_type; 			// firt template parameter
	typedef Alloc										allocator_type; 		// second template parameter
	typedef T&											reference; 				// A type provides a reference to an element stored in a vector.
	typedef const T&									const_reference;		// Type that give a pointer to an element stored.
	typedef T*											pointer;				// Type that give a const pointer to an element stored.
	typedef const T*									const_pointer;			// A random access iterator to value_type
	typedef random_access_iterator<T>		 			iterator;				// A random access iterator to const value_type
	typedef random_access_iterator<const T> 			const_iterator;			// Used to iterate through the vector in reverse.
	typedef reverse_iterator<const_iterator>			const_reverse_iterator;	// Can represent the difference between iterators to the element actually stored.
	typedef reverse_iterator<iterator>					reverse_iterator;		// That can read any element in a reversed the vector.
	typedef std::ptrdiff_t								difference_type;		// A signed integral type, can represent the difference between iterators
...
private:
	...
	{ // reallocate with new size and a new value
	...
	{ // assign a value other vector until fill _size/ reallocate with new size
		while (_size < n) {
